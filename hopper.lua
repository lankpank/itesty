...
-- ============ EVENT PURCHASE FUNCTION ============
local function purchaseEvents()
    ...
    -- Purchase Mythic Event
    print("🛒 Purchasing Mythic Event...")
    remoteEvent:FireServer("PurchaseEventBoard", "Mythic Event")
    task.wait(1)
    
    -- Purchase Secret Event
    print("🛒 Purchasing Secret Event...")
    remoteEvent:FireServer("PurchaseEventBoard", "Secret Event")
    task.wait(1)
    
    -- Purchase Infinity Event
    print("🛒 Purchasing Infinity Event...")
    remoteEvent:FireServer("PurchaseEventBoard", "Infinity Event")
    
    print("✅ All events purchased!")
end
...
local function teleportToRift()
    ...
    if distance < 20 then
        ...
        purchaseEvents() -- Purchase events after confirming we're at the rift
        return true
    end
    ...
    local success = pcall(function()
        ...
        purchaseEvents() -- Purchase events after successful teleport
    end)
    ...
    if not success then
        ...
        if humanoid then
            pcall(function()
                ...
                task.wait(2)
                purchaseEvents() -- Purchase events after moving
            end)
        end
        return false
    end
